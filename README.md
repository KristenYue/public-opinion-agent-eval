...
> 旧 XGBoost 在二次复核测试子集上的 Accuracy 为 44.7%，SnowNLP 为 47.4%；因此二者只作为基线信号，不满足生产级自动判定条件。旧切分上 83.9% 的 Transformer 结果不作为跨事件泛化声明。
...
打开 `http://127.0.0.1:7860`，点击“加载示例”后运行，即可查看检索证据、风险结论、复核状态与审计轨迹。若触发复核，可在人工复核工作台修改最后一列标签，再点击“应用人工复核并重新生成简报”；状态会更新为 `human_completed`。
